...
module OpenMayFail where
  ...
  -- Return and bind as 'usual'
  ret : ∀ {a : State -> Set} {s : State} -> a s -> m a s
  ret = Stop
  ...
  _⟫=_ :  ∀ {a b : State -> Set} {s : State} ->
    m a s -> ((xy : Sigma State a) -> m b (fst xy)) -> m b s
  _⟫=_ c f = c >>= (\ {s} x -> f (s , x))

  return : ∀ {a : Set} {s : State} -> a -> m (At s a) s
  return {s = s} x = Stop (V x)

  pattern at s x = (s , (V x))
  pattern opened x = (Open , (V x))
  pattern closed x = (Closed , (V x))  

  -- now we only have to worry about the state that we're in
  -- the pattern matching lambdas and implicits are ugly...
  -- can this be improved?
  not-stuck : m (At Closed (Maybe Char)) Closed
  not-stuck = openFile ⟫= (\ { (at Closed x) → return Nothing ;
                               (at Open x) → read ⟫= \ { (opened c)  ->
                                             closeFile ⟫= (\ {(closed tt) ->
                                             return c})}})
